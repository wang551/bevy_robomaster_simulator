...
- Platform-specific code exists for macOS (MetalFX via objc2-metal in `src/metalfx.rs` and Cargo.toml) and WSL (wgpu non-compliant adapter workaround in `src/main.rs`). The ros2 feature needs a sourced ROS2 environment; it builds on Linux and on Windows (see BUILD.md for the verified Windows/pixi-lyrical workflow).
